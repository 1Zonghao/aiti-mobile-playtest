...
export function ConferencePoster() {
  ...
  return <main className="poster-page">
    ...
    <section className="poster-qr" aria-label="大会测试二维码">
      {site.status === "available" ? <><QRCodeSVG value={site.url} size={280} bgColor="#fffdf7" fgColor="#173f35" level="H" /><p>{site.url}</p></> : <div className="poster-config"><strong>二维码尚未生成</strong><p>请配置 NEXT_PUBLIC_SITE_URL 后重新构建。</p></div>}
    </section>
