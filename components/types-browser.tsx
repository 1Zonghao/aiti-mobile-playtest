...
export function TypesBrowser({ types }: { types: ResultType[] }) {
  ...
  return <>
    ...
    <div className="types-grid">{visible.map((type) => <article className="type-card panel" key={type.code}><CharacterVisual result={type} compact /><p className="label mt-5 mb-0">AITI {type.code}</p><h2>{type.name}</h2><p>{type.definition}</p><Link className="button secondary" href={`/types/${type.code.toLowerCase()}`}>探索类型</Link></article>)}</div>
  </>;
}
