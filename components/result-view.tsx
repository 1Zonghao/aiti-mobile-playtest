    ...
    <main>
      <article className="result-sheet" aria-label={`${result.code} ${result.name}结果档案`}>
        ...
        <section className="result-block min-h-[80dvh] flex flex-col justify-center">
          ...
          {paperUrl && <div className="mt-5 grid grid-cols-[auto_1fr] items-center gap-5 border-2 border-dashed border-[var(--rule)] bg-[var(--paper)] p-5"><a href={paperUrl} target="_blank" rel="noopener noreferrer"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(paperUrl)}`} alt="论文二维码" width={100} height={100} style={{ display: "block" }} /></a><div><p className="label m-0 mb-2">📄 论文原文</p><p className="m-0 text-sm leading-6 font-bold"><a href={paperUrl} target="_blank" rel="noopener noreferrer" className="no-underline hover:underline">《爱的是她，还是被算法过拟合的你自己》</a></p><p className="m-0 mt-1 text-xs text-[var(--muted)] leading-5">点击二维码或标题即可打开论文。长按二维码可保存图片。</p></div></div>}
